...
import com.farmerworking.db.rabbitDb.impl.OverflowUtils;
...
public final class Crc32C implements Checksum {
    ...
    public static int extend(int initCrc, String append) {
        Crc32C crc32c = new Crc32C();
        crc32c.setIntValue(initCrc);

        char[] chars = append.toCharArray();
        byte[] bytes = new byte[chars.length];
        for (int i = 0; i < chars.length; i++) {
            bytes[i] = (byte) OverflowUtils.unsignedInt8Overflow(chars[i]);
        }
    ...
    private void setIntValue(int crcIntValue) {
        this.crc = ~crcIntValue;
    }
